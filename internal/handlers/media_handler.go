...
func (h *MediaHandler) BulkUpload(c *gin.Context) {
	var req []models.MediaCreateRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request", err.Error())
		return
	...
	var uploadedMedia []models.MediaResponse
	for _, mediaReq := range req {
		media, err := h.mediaService.Create(&mediaReq, uploaderID)
		if err != nil {
			response.InternalServerError(c, err.Error())
			return
		}
	...
	response.Created(c, "Media files uploaded successfully", uploadedMedia)
}
...
func (h *MediaHandler) Update(c *gin.Context) {
	...
	media, err := h.mediaService.Update(uint(id), &req)
	if err != nil {
		response.InternalServerError(c, err.Error())
		return
	}

	response.Success(c, "Media file updated successfully", media)
}

func (h *MediaHandler) GetUsage(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil {
		response.BadRequest(c, "Invalid ID", err.Error())
		return
	...
	response.Success(c, "Media usage retrieved successfully", usage)
}
