...
func (h *EventFlyerHandler) Reorder(c *gin.Context) {
	var req models.EventFlyerReorderRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request", err.Error())
		return
	}

	if err := h.eventFlyerService.Reorder(&req); err != nil {
		response.InternalServerError(c, err.Error())
		return
	}

	response.Success(c, "Event flyers reordered successfully", nil)
}

func (h *EventFlyerHandler) ToggleStatus(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil {
		response.BadRequest(c, "Invalid ID", err.Error())
		return
	...
	// Toggle status
	newStatus := !flyer.IsActive
	updateReq := &models.EventFlyerUpdateRequest{
		IsActive: &newStatus,
	}

	updatedFlyer, err := h.eventFlyerService.Update(uint(id), updateReq)
	if err != nil {
		response.InternalServerError(c, err.Error())
		return
	}

	response.Success(c, "Event flyer status toggled successfully", updatedFlyer)
}
